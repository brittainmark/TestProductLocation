...
class ScriptedInstaller extends ScriptedInstallBase
{
    ...
    protected function executeInstall()
    {
        ...
        IF (defined('VS_PRODUCTS_LOCATION_VERSION')) {
          ...
          ("Version", "VS_PRODUCTS_LOCATION_VERSION", "0.0.0", "Version installed:", ' . $gid . ', 0, NOW(), NOW(), NULL, "zen_cfg_select_option([\'0.0.0\'],");';
        }
         ...
         * register page
         */
        zen_register_admin_page('configVSProductLocation', 'BOX_CONFIGURATION_VS_PRODUCTS_LOCATION', 'FILENAME_CONFIGURATION', 'gID=' . $gid, 'configuration', 'Y', $gid);
        /*
         * Create table to hold product locations ?and initialise with empty values?
         */
        if(!defined('TABLE_VS_PRODUCTS_LOCATION')) {
            require dirname(__FILE__) . '/../admin/includes/extra_datafiles/vs_products_location.php'; 
        }
...
//        $this->executeInstallerSql($sql);
        ...
        $sql = "INSERT IGNORE INTO " . TABLE_CONFIGURATION . "
                ...
                set_function )
                VALUES
                ( $gid, 10, NOW(), 'VS_PRODUCTS_LOCATION_PLUGIN_CHECK', 'Very Simple Product Location (Update Check)', 'false', 'Allow version checking if Zen Cart version checking enabled<br><br>If false, no version checking performed.<br>If true, then only if Zen Cart version checking is on:', NULL, 'zen_cfg_select_option([\'true\', \'false\'],'),
                ( $gid, 20, NOW(), 'VS_PRODUCTS_LOCATION_VERSION', 'Very Simple Product Location<b> Version</b>', '1.0.0', 'Very Simple Product Location Version', NULL, 'zen_cfg_select_option([\'1.0.0\'],'),
                ( $gid, 30, NOW(), 'VS_PRODUCTS_LOCATION_PACKING_DISPLAY', 'Display location on Packing Slip', 'true',  '<br>If true, the products location will be displayed on the Packing Slip.<br><b>Default: true</b>', NULL, 'zen_cfg_select_option([\'true\', \'false\'],'),
                ( $gid, 40, NOW(), 'VS_PRODUCTS_LOCATION_INVOICE_DISPLAY', 'Display location on invoice',  'true', '<br>If true, the products location will be displayed on the invoice.<br><b>Default: true</b>', NULL, 'zen_cfg_select_option([\'true\', \'false\'],'),
                ( $gid, 50, NOW(), 'VS_PRODUCTS_LOCATION_CATEGORY_DISPLAY', 'Display location on catalog products category', 'true', '<br>If true, the products location will be displayed on the catalog products categories display when desplaying lists of products.<br><b>Default: true</b>', NULL, 'zen_cfg_select_option([\'true\', \'false\'],'),
                ( $gid, 60, NOW(), 'VS_PRODUCTS_LOCATION_SORT_DISPLAY', 'Sort Invoice and packing slipt by location', 'true', '<br>If true, the Invoice and the Picking list will be sorted by products location.<br><b>Default: true</b>', NULL, 'zen_cfg_select_option([\'true\', \'false\'],')";
